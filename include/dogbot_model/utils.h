...
inline Eigen::MatrixXd toEigenKDL(const std::vector<Eigen::Matrix<double,3,24>>& J)
{
    Eigen::Matrix<double,12,24> Ja;
    for (int i = 0; i < J.size(); i ++)
    {
        Ja.block(i*3,0,3,24) = J.at(i);
    }
    return Ja;
}

inline Eigen::Vector3d toEigenKDL(const KDL::Vector& v)
{
    return Eigen::Vector3d(v.x(),v.y(),v.z());
}

inline Eigen::Matrix3d toEigenKDL(const KDL::Rotation& M)
{
    Eigen::Matrix3d E;
...
inline Eigen::Matrix<double,6,1> toEigenKDL(const KDL::Wrench& w)
{
    Eigen::Matrix<double,6,1> e;
    e << toEigenKDL(w.force), toEigenKDL(w.torque);
    return e;
}

inline Eigen::Matrix<double,6,1> toEigenKDL(const KDL::Twist& t)
{
    Eigen::Matrix<double,6,1> e;
    e << toEigenKDL(t.vel), toEigenKDL(t.rot);
    return e;
}


inline Eigen::VectorXd toEigenKDL(alglib::real_1d_array& v)
{
...
inline Eigen::Matrix<double,6,6> spatialRotation(const KDL::Rotation& M)
{
    ...
    R.block(0,0,3,3) = toEigenKDL(M);
    R.block(3,3,3,3) = toEigenKDL(M);
...
inline KDL::Jacobian adjoint(const KDL::Frame & F, const KDL::Jacobian &_J)
{
    ...
    ad.setZero();
    ad.block(0,0,3,3) = toEigenKDL(F.M);
    ad.block(0,3,3,3) = skew(toEigenKDL(F.p))*toEigenKDL(F.M);;
    ad.block(3,3,3,3) = toEigenKDL(F.M);;
...
inline KDL::Twist adjoint(const KDL::Frame & F, const KDL::Twist &T)
{
    ...
    t << toEigenKDL(T.vel), toEigenKDL(T.rot);
...
{
//    std::cout << toEigenKDL(_Fd.p) << std::endl;
//    std::cout << toEigenKDL(_Fe.p) << std::endl;
//    std::cout << toEigenKDL(_Fd.M) << std::endl;
//    std::cout << toEigenKDL(_Fe.M) << std::endl;

    Eigen::Matrix<double,3,3,Eigen::RowMajor> R_d(_Fd.M.data);
    Eigen::Matrix<double,3,3,Eigen::RowMajor> R_e(_Fe.M.data);
    e << computeLinearError(toEigenKDL(_Fd.p), toEigenKDL(_Fe.p)), computeOrientationError(R_d, R_e);
    edot << computeLinearError(toEigenKDL(_Vd.vel), toEigenKDL(_Ve.vel)), -toEigenKDL(_Ve.rot);//computeOrientationVelocityError(toEigenKDL(_Vd.rot), toEigenKDL(_Ve.rot), R_d, R_e);
}
