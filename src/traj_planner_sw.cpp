...
TrajPlannerSw::TrajPlannerSw(double ti_, double tf_, 
                        Eigen::Matrix<double,3,1> &init_pos,
                        Eigen::Matrix<double,3,1> &end_pos,
                        Eigen::Matrix<double,3,1> &init_vel,
                        Eigen::Matrix<double,3,1> &end_vel,
                        Eigen::Matrix<double,3,1> &init_acc,
                        Eigen::Matrix<double,3,1> &end_acc)
{
    // Initial time
      ti=ti_;
...
{   
     ...
     Eigen::Matrix<double,6,6> Ax;
      ...
      Ax<< pow(ti,5), pow(ti,4), pow(ti,3), pow(ti,2), ti, 1,
           ...
           pow(tf/2,5), pow(tf/2,4), pow(tf/2,3), pow(tf/2,2), tf/2, 1,
           5*pow(tf/2,4), 4*pow(tf/2,3), 3*pow(tf/2,2), 2*(tf/2), 1, 0,
           20*pow(tf/2,3), 12*pow(tf/2,2), 6*tf/2, 2, 0, 0;
     ...
     by<<-0.276, initvel(1,0), initacc(1,0), -0.276-0.02, endvel(1,0),endacc(1,0);
     ...
     bz<<0.0279, 0.025, initacc(2,0), 0.0379, 0.0,endacc(2,0);
     ...
     {   double tn=0.001*i;
      ...
      Ax<< pow(tf/2,5), pow(tf/2,4), pow(tf/2,3), pow(tf/2,2), tf/2, 1,
           5*pow(tf/2,4), 4*pow(tf/2,3), 3*pow(tf/2,2), 2*tf/2, 1, 0,
           20*pow(tf/2,3), 12*pow(tf/2,2), 6*tf/2, 2, 0, 0,
     ...
     // X coefficient
      coeffx=Ax.inverse()*bx;


  

     by<<-0.276-0.02, initvel(1,0), initacc(1,0), -0.276-0.04, endvel(1,0),endacc(1,0);
     
     // Y coefficient
        coeffy=Ax.inverse()*by;

     

     bz<<0.0379, 0.0, initacc(2,0), 0.027, -0.02,endacc(2,0);
    

     // Z coefficient
         coeffz=Ax.inverse()*bz;
...
trajectory_pointsw TrajPlannerSw::getTraj()
{
    return traj;
